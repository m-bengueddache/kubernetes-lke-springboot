...
**`b64enc` pour les secrets** : les valeurs sont passées en clair dans le fichier de valeurs — Helm les encode en base64 au moment du rendu. Pas de `echo -n "..." | base64` manuel.
...
**Guards `enabled`** : `Secret`, `ConfigMap` et `Ingress` sont rendus uniquement pour la release java-app (`enabled: true`). La release phpmyadmin les désactive (`enabled: false`) et réutilise les ressources déjà créées.
...
**`b64enc` for secrets**: values are passed as plain text in the values file — Helm encodes them to base64 at render time. No manual `echo -n "..." | base64`.
...
**`enabled` guards**: `Secret`, `ConfigMap` and `Ingress` are rendered only for the java-app release (`enabled: true`). The phpmyadmin release disables them (`enabled: false`) and reuses the already-created resources.
